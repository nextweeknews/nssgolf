...
async function runImport() {
  ...
  if (error) {
    if (isMissingAliasTable(error)) {
      throw new Error(`Import failed: ${error.message}. Apply the repository Supabase migrations first.`);
    }
    throw error;
  }
...
async function runExportSql() {
  ...
  const rowsToUpsert = [
    ...approvedRows.map((row) => ({ row, active: true })),
    ...skippedRows.map((row) => ({ row, active: false })),
  ];

  const values = rowsToUpsert.map(({ row, active }) => {
    const leagueKey = row.league_key || defaultLeagueKey;
    const guildId = row.guild_id || getArg("--guild-id", process.env.DISCORD_GUILD_ID || "");
    const leaguePlayerName = String(row.league_player_name || "").trim();
    const discordUserId = active
      ? String(row.approved_discord_user_id || row.suggested_discord_user_id || "").trim()
      : "";
    const notes = row.notes || (active ? null : "Skipped during Pro League alias review");
    ...
    if (active && !discordUserId) throw new Error(`Approved row for '${leaguePlayerName}' is missing approved_discord_user_id or suggested_discord_user_id.`);

    return `  (${sqlString(leagueKey)}, ${sqlString(leaguePlayerName)}, ${sqlString(guildId)}, ${sqlString(active ? discordUserId : null)}, ${active ? "true" : "false"}, 'proleague-alias-script', ${sqlString(notes)})`;
  });
...
-- Run only after applying the repository Supabase migrations.
-- Approved rows are upserted with active = true.
-- Blank/unapproved rows are preserved with active = false and no Discord ID.

begin;
${schemaCompatibilitySql}${upsertSql}
...
`;
  ...
  console.log(`Wrote SQL for ${approvedRows.length} approved mappings and ${skippedRows.length} inactive skipped mappings to ${sqlPath}`);
}
